...
def test_hover():
    ...
    sim = Simulator(dt=0.01, mass=120.0)
...
def test_step_altitude():
    ...
    def cmd_func(t):
        ...
        return {'pos_neu': np.array([0.0, 0.0]), 'yaw': 0.0, 'alt': alt}
...
def test_step_position():
    ...
    def cmd_func(t):
        ...
        return {'pos_neu': pos, 'yaw': 0.0, 'alt': 10.0}

    logs = sim.run(cmd_func, duration=20.0, log_interval=0.01)

    final_pos = logs['pos_neu'][-1, :2]
...
def test_step_yaw():
    ...
    def cmd_func(t):
        ...
        return {'pos_neu': np.array([0.0, 0.0]), 'yaw': yaw, 'alt': 10.0}
